...
class Weglot {
	...
	public function plugin_menu() {
		...
		if(isset($_GET['settings-updated']) && $_GET['settings-updated'] && strpos($this->request_uri, 'page=Weglot') !== false)
		{
			...
			$accepted = array("sq","en","ar","hy","az","af","eu","be","bg","bs","vi","hu","ht","nl","el","ka","da","he","id","ga","it","is","es","kk","ca","ky","zh","ko","lv","lt","mg","ms","mt","mk","mn","de","no","fa","pl","pt","ro","ru","sr","sk","sl","sw","tg","th","tr","uz","uk","fi","fr","hr","cs","sv","et","ja","hi","ur");
			foreach($d as $k=>$l) {
	...
	public function treatPage($final) {
		...
		if(!is_admin() && strpos($request_uri,'wc-ajax') === false && $this->original_l!="" && $this->destination_l!="") {
			...
			if($this->isEligibleURL($url) && WGUtils::is_HTML($final)) {
				...
				if (strpos($final,'<div id="weglot_here"></div>') !== false) {
					...
					$final = WGUtils::str_lreplace('<div id="weglot_here"></div>',$button,$final);
				}
		...
		elseif((strpos($request_uri,'admin-ajax.php') !== false || strpos($request_uri,'?wc-ajax') !== false) && $this->destination_l!="" && $this->original_l!="" && strpos($_SERVER["HTTP_REFERER"],'admin') === false ) {
